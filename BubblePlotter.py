...
for circ in circs_list:
    ...
    size=int(circ[-1])/2
    size=(((size**2)*3.14159265)/int(totcov[-1].split("\t")[1]))*12
    if circ[4] <= 0 :
        if circ[5] > 0.6:
            plt.scatter((int(circ[2])+int(circ[1]))/2,int(circ[0]),size,c='black',linewidth=0,alpha=0.5)
            
        else :
            plt.scatter((int(circ[2])+int(circ[1]))/2,int(circ[0]),size,c="darkorange",linewidth=0,alpha=0.5)
    else :
        if circ[5] > 0.6:
            plt.scatter((int(circ[2])+int(circ[1]))/2,-(int(circ[0])),size,c='black',linewidth=0,alpha=0.5)
        else :
...
plt.savefig("./Circ_List_plot_{name}.png".format(name=str(totcov[-1].split("\t")[0])),dpi=1000,bbox_inches='tight')
